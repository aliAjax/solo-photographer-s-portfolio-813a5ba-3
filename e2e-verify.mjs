#!/usr/bin/env node
/**
 * 自带的真实浏览器核验脚本 —— 无需手工先启动应用。
 *
 * 脚本会自行：
 *   1. 以独立端口（--strictPort）启动 Vite dev server；
 *   2. 等待应用就绪后跑完全部交互核验；
 *   3. 无论成功或失败都回收服务进程。
 *
 * 前置：已在应用根目录执行 `npm install`，并安装过 Chromium
 * （`npx playwright install chromium`）。
 *
 * 覆盖 task.md「验证要求」：
 *   1) 五条路由均可渲染且控制台无 error
 *   2) /work 四个筛选项；筛选 → 进入系列 → 返回，筛选保持
 *   3) 灯箱在首页 / 作品集 / 系列页复用；筛选子集内循环（4 张回到首张），显示位置与说明
 ...
 *   8) 系列详情与 photos.json 共享同一数据模型，顺序一致
 *
 * 截图产物写入 e2e-results/（可复查，已 gitignore）。
 */
...
const data = JSON.parse(fs.readFileSync(path.join(ROOT, "src/data/photos.json"), "utf8"));

/** 自启动 Vite（直接运行 vite.js，避免依赖 npm/shell），轮询等待就绪。 */
function startServer() {
  const viteBin = path.join(ROOT, "node_modules", "vite", "bin", "vite.js");
  if (!fs.existsSync(viteBin)) {
    throw new Error("未找到 node_modules/vite，请先在应用根目录执行 npm install");
  }
  const child = spawn(
    process.execPath,
    [viteBin, "--port", String(PORT), "--host", HOST, "--strictPort"],
  ...
  const ready = new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      if (!settled) {
        settled = true;
        reject(new Error(`等待 ${BASE} 就绪超时（30s）`));
      }
    }, 30_000);
    ...
    child.on("exit", (code) => {
      if (!settled) {
        settled = true;
        clearTimeout(timer);
        reject(new Error(`Vite 进程提前退出（code=${code}）`));
      }
    });
  });
...
async function main() {
  ...
  try {
    try {
      ...
      throw new Error(
        `无法启动 Chromium（${e.message}）。请先执行：npx playwright install chromium`,
      );
    }
    ...
    // ---------- 3a. 灯箱：三入口同一组件 ----------
    test("灯箱在 /work、系列页、首页三处复用，显示位置与说明", async () => {
      const page = await browser.newPage({ viewport: { width: 1440, height: 1000 } });
      await page.goto(`${BASE}/work`, { waitUntil: "networkidle" });
    ...
    test("390px 窄屏：作品集单列 + 汉堡菜单 + 灯箱说明置底", async () => {
      ...
      await page.screenshot({ path: path.join(OUT, "lightbox-mobile.png") });
      await page.close();
    });
    ...
    test("联系表单：禁用态/行内错误/成功态", async () => {
      ...
      await page.screenshot({ path: path.join(OUT, "contact-success.png") });
      await page.close();
    });
    ...
    console.log(`\n${passed}/${tests.length} 通过，截图产物：${path.relative(ROOT, OUT)}/`);
  } finally {
    await browser?.close();
    server.stop();
  }
